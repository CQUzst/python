...
for i in range(1,len(x)):
    ...
    real_time_y=y[i]
    # 一阶指数平滑
    s_x.append(alpha * real_time_x + (1 - alpha) * (s_x[i-1]+ gama * t_x[i - 1]))
    s_y.append(alpha * real_time_y + (1 - alpha) * (s_y[i-1]+ gama * t_y[i - 1]))

    t_x.append(beta * (s_x[i] - s_x[i - 1]) + (1 - beta) * gama * t_x[i - 1])
    t_y.append(beta * (s_y[i] - s_y[i - 1]) + (1 - beta) * gama * t_y[i - 1])

    sum_trend = 0
    for i in range(1,len(t_x)):
        sum_trend+=gama**i
    predict_x.append(s_x[i] +sum_trend * t_x[i])
    predict_y.append(s_y[i] +sum_trend * t_y[i])
