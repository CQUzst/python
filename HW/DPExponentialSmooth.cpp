...
x_ex_one=[]
y_ex_one=[]
x_ex_one.append(x_double_exponential[0])
y_ex_one.append(y_double_exponential[0])
for i in range(1,len(x)):
    ...
    x_ex_one.append(alpha2 * x_double_exponential[i] + (1 - alpha2) * x_ex_one[i - 1])  # 一阶指数平滑x
    y_ex_one.append(alpha2 * y_double_exponential[i] + (1 - alpha2) * y_ex_one[i - 1])  # 一阶指数平滑y
    plt.clf()
...
plt.plot(x_list_double, y_list_double, color='g', linestyle='-', marker='.', label=u"二次指数平滑")
plt.plot(x_list_double, y_list_double, color='g', linestyle='-', marker='.', label=u"二次指数平滑")
plt.plot(x_double_exponential, y_double_exponential, color='b', linestyle='-', marker='.', label=u"二阶指数平滑")
plt.plot(x_ex_one, y_ex_one, color='m', linestyle='-', marker='.', label=u"二阶指数+一次平滑")
